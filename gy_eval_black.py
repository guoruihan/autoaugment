...
def train_child(t, p, m, load_dict=False):
    ...
    if load_dict:
        ...
        raw_optimizer.load_state_dict(torch.load('black_eval_runs/raw_optimizer.pt'))
    model.train()
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(0, len(val_x_adv), BATCH_SIZE), desc='Noise: ', leave=False):
        ...
        val_x_adv = np.zeros_like(val_x)
        for i in tqdm(range(0, len(val_x_adv), BATCH_SIZE), desc='Noise: ', leave=False):
            val_x_adv[i:][:BATCH_SIZE] = noise_raw_op(val_x[i:][:BATCH_SIZE], 0.3)
        adv_valset = torch.utils.data.TensorDataset(torch.tensor(val_x_adv, dtype=torch.float), torch.tensor(val_y, dtype=torch.long))
        adv_valloader = torch.utils.data.DataLoader(adv_valset, batch_size=BATCH_SIZE, shuffle=False, num_workers=4)
        batch_tqdm = tqdm(adv_valloader, leave=False)
        tot_acc = 0
...
load_dict = os.path.isfile('black_eval_runs/model.pt')
if not load_dict:
    with open('black_eval_runs/acc.csv', 'w') as f:
        pass
